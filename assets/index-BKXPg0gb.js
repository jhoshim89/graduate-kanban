...
- 탭(일상/개발)은 위쪽 '+ 탭 추가'로 얼마든지 만들 수 있어요.`,h="__all__";class Z{constructor(){this.tasksKey="graduate_kanban_tasks_v2",this.viewKey="graduate_kanban_view_v2",this.memoKey="graduate_kanban_memo_v2",this.catKey="graduate_kanban_categories_v2",this.activeCatKey="graduate_kanban_active_category_v2";const t=localStorage.getItem(this.tasksKey);let s=t?JSON.parse(t):X;s=s.map(i=>{if(i.category)return i;const f=_[i.owner]||"일상";return{...i,category:f}});const a=localStorage.getItem(this.catKey);let n=a?JSON.parse(a):null;if(!n||!Array.isArray(n)||n.length===0){const i=[...new Set(s.map(f=>f.category).filter(Boolean))];n=[...new Set([...i,...V])]}const r=localStorage.getItem(this.viewKey)||"board",o=r.startsWith("board")?"board":r,c=localStorage.getItem(this.activeCatKey),d=c&&(c===h||n.includes(c))?c:h;this.state={tasks:s,categories:n,activeCategory:d,currentView:o,currentMonth:new Date,memo:localStorage.getItem(this.memoKey)||O},t||(this.saveTasksToLocalStorage(),localStorage.setItem(this.memoKey,O)),this.saveTasksToLocalStorage(),this.saveCategoriesToLocalStorage(),this.listeners=[]}saveTasksToLocalStorage(){localStorage.setItem(this.tasksKey,JSON.stringify(this.state.tasks))}saveViewToLocalStorage(){localStorage.setItem(this.viewKey,this.state.currentView)}saveCategoriesToLocalStorage(){localStorage.setItem(this.catKey,JSON.stringify(this.state.categories))}saveActiveCategoryToLocalStorage(){localStorage.setItem(this.activeCatKey,this.state.activeCategory)}getTasks(){return[...this.state.tasks]}getCategories(){return[...this.state.categories]}getActiveCategory(){return this.state.activeCategory}getVisibleTasks(){return this.state.activeCategory===h?[...this.state.tasks]:this.state.tasks.filter(t=>t.category===this.state.activeCategory)}getCurrentView(){return this.state.currentView}getCurrentMonth(){return this.state.currentMonth}getMemo(){return this.state.memo}setView(t){["board","calendar","dashboard"].includes(t)&&(this.state.currentView=t,this.saveViewToLocalStorage(),this.notify())}setActiveCategory(t){t!==h&&!this.state.categories.includes(t)||(this.state.activeCategory=t,this.saveActiveCategoryToLocalStorage(),this.notify())}addCategory(t){const s=(t||"").trim();return s?s===h||s==="전체"?{ok:!1,message:"'전체'는 기본 탭이라 추가할 수 없어요."}:this.state.categories.includes(s)?{ok:!1,message:"이미 있는 탭이에요."}:(this.state.categories.push(s),this.saveCategoriesToLocalStorage(),this.state.activeCategory=s,this.saveActiveCategoryToLocalStorage(),this.notify(),{ok:!0}):{ok:!1,message:"탭 이름을 입력해 주세요."}}deleteCategory(t){if(!this.state.categories.includes(t))return{ok:!1};if(this.state.categories.length<=1)return{ok:!1,message:"탭은 최소 한 개는 있어야 해요."};this.state.categories=this.state.categories.filter(a=>a!==t);const s=this.state.categories[0];return this.state.tasks=this.state.tasks.map(a=>a.category===t?{...a,category:s}:a),this.state.activeCategory===t&&(this.state.activeCategory=h,this.saveActiveCategoryToLocalStorage()),this.saveCategoriesToLocalStorage(),this.saveTasksToLocalStorage(),this.notify(),{ok:!0,movedTo:s}}renameCategory(t,s){const a=(s||"").trim();return a?this.state.categories.includes(t)?a===t?{ok:!0}:this.state.categories.includes(a)?{ok:!1,message:"이미 있는 탭 이름이에요."}:(this.state.categories=this.state.categories.map(n=>n===t?a:n),this.state.tasks=this.state.tasks.map(n=>n.category===t?{...n,category:a}:n),this.state.activeCategory===t&&(this.state.activeCategory=a,this.saveActiveCategoryToLocalStorage()),this.saveCategoriesToLocalStorage(),this.saveTasksToLocalStorage(),this.notify(),{ok:!0}):{ok:!1}:{ok:!1,message:"탭 이름을 입력해 주세요."}}setCalendarMonth(t){this.state.currentMonth=t,this.notify()}saveMemo(t){this.state.memo=t,localStorage.setItem(this.memoKey,t)}addTask(t,s="",a=null){const n=a||(this.state.activeCategory!==h?this.state.activeCategory:this.state.categories[0]||"일상"),r={id:`task-${Date.now()}-${Math.random().toString(36).substr(2,9)}`,title:t.trim(),dueDate:s||"",status:"todo",category:n};return this.state.tasks.push(r),this.saveTasksToLocalStorage(),this.notify(),r}updateTask(t,s){this.state.tasks=this.state.tasks.map(a=>a.id===t?{...a,...s}:a),this.saveTasksToLocalStorage(),this.notify()}updateTaskStatus(t,s){["todo","inprogress","done"].includes(s)&&this.updateTask(t,{status:s})}deleteTask(t){this.state.tasks=this.state.tasks.filter(s=>s.id!==t),this.saveTasksToLocalStorage(),this.notify()}exportData(){return JSON.stringify({_app:"graduate-kanban",_version:3,tasks:this.state.tasks,categories:this.state.categories,memo:this.state.memo,currentView:this.state.currentView},null,2)}importData(t){let s;try{s=JSON.parse(t)}catch{return{ok:!1,message:"파일을 읽을 수 없어요. 올바른 백업 파일인지 확인해 주세요."}}if(!s||!Array.isArray(s.tasks))return{ok:!1,message:"이 칸반 백업 파일이 아닌 것 같아요."};const a=s.tasks.map(r=>r.category?r:{...r,category:_[r.owner]||"일상"});let n;if(Array.isArray(s.categories)&&s.categories.length>0)n=[...new Set(s.categories)];else{const r=[...new Set(a.map(o=>o.category).filter(Boolean))];n=[...new Set([...r,...V])]}return this.state.tasks=a,this.state.categories=n,n.includes(this.state.activeCategory)||(this.state.activeCategory=h,this.saveActiveCategoryToLocalStorage()),typeof s.memo=="string"&&(this.state.memo=s.memo),this.saveTasksToLocalStorage(),this.saveCategoriesToLocalStorage(),localStorage.setItem(this.memoKey,this.state.memo),this.notify(),{ok:!0,count:a.length}}subscribe(t){return this.listeners.push(t),()=>{this.listeners=this.listeners.filter(s=>s!==t)}}notify(){this.listeners.forEach(t=>t(this.state))}}const u=new Z;function P(e){if(!e)return{class:"due-none",text:"기한 없음"};const t=new Date;t.setHours(0,0,0,0);const s=new Date(e);s.setHours(0,0,0,0);const a=s-t,n=Math.ceil(a/(1e3*60*60*24));if(n<0)return{class:"due-overdue",text:`지연 D+${Math.abs(n)}`};if(n===0)return{class:"due-overdue",text:"오늘"};if(n===1)return{class:"due-soon",text:"내일"};if(n===2)return{class:"due-soon",text:"모레"};{const r=e.split("-");return{class:"due-normal",text:`${r.length===3?`${parseInt(r[1])}/${parseInt(r[2])}`:e} (D-${n})`}}}function Q(){const e=u.getCategories(),t=u.getActiveCategory();return`
  ...
  `}function j(e,t){const s=e.status==="done",a=P(e.dueDate),n=s?"task-card-title inline-edit-title line-through":"task-card-title inline-edit-title",r=s?"text-mute":a.class,o=s?"check-circle-2":"circle",c=s?"text-violet":"text-mute",d=A(e.title),i=t&&e.category?`<span class="card-category-badge">${A(e.category)}</span>`:"";return`
    <div class="task-card card-reminder ${s?"is-done":""}" draggable="true" data-id="${e.id}">
      ...
      <div class="reminder-right-slot">
        <button class="card-move-btn" data-id="${e.id}" title="다른 탭으로 옮기기" aria-label="다른 탭으로 옮기기">
  ...
  `}function tt(){const e=u.getVisibleTasks(),t=u.getActiveCategory()===h,s=e.filter(r=>r.status!=="done"),a=e.filter(r=>r.status==="done"),n=s.length===0&&a.length===0?'<div class="empty-state list-empty"><span>이 탭에 할 일이 없어요. 아래에 입력해서 추가하세요.</span></div>':"";return`
    ${Q()}
  ...
  `}function A(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function et(){const e=u.getCurrentMonth(),t=e.getFullYear(),s=e.getMonth(),a=u.getTasks(),n=new Date(t,s,1).getDay(),r=new Date(t,s+1,0).getDate(),o=new Date(t,s,0).getDate(),c=new Date;c.setHours(0,0,0,0);const d=[];for(let p=n-1;p>=0;p--){const v=new Date(t,s-1,o-p);d.push({date:v,isCurrentMonth:!1,dayNumber:o-p})}for(let p=1;p<=r;p++){const v=new Date(t,s,p),C=v.toDateString()===c.toDateString();d.push({date:v,isCurrentMonth:!0,isToday:C,dayNumber:p})}const f=42-d.length;for(let p=1;p<=f;p++){const v=new Date(t,s+1,p);d.push({date:v,isCurrentMonth:!1,dayNumber:p})}return`
    <div class="calendar-view">
      ...
      <div class="calendar-grid">
        ${d.map(p=>{const v=p.date.toISOString().split("T")[0],C=a.filter(m=>m.dueDate===v);return`
  ...
  `}function k(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function E(e){if(!e)return null;e=e.trim().toLowerCase();const t=new Date;if(t.setHours(0,0,0,0),/^(오늘|금일|today|d-day|dday)$/.test(e))return y(t);if(/^(내일|명일|tomorrow)$/.test(e))return y(x(t,1));if(/^(모레|내일모레|글피)$/.test(e))return y(x(t,2));if(/^(어제|작일|yesterday)$/.test(e))return y(x(t,-1));let s=e.match(/^(\d+)\s*일\s*(뒤|후|후에|이후)$/);if(s||(s=e.match(/^(\d+)\s*days?\s*(after|later)$/),s))return y(x(t,parseInt(s[1])));if(s=e.match(/^(\d+)\s*일\s*(전|전에|이전)$/),s)return y(x(t,-parseInt(s[1])));if(s=e.match(/^(이번주|다음주)?\s*(월|화|수|목|금|토|일)요일?$/),s){const n=s[1]==="다음주",r=s[2],c=["일","월","화","수","목","금","토"].indexOf(r),d=t.getDay();let i=c-d;return(n||i<=0)&&(i+=7),y(x(t,i))}if(s=e.match(/^(\d{1,2})\s*[월/\-.]\s*(\d{1,2})\s*일?$/),s){const n=parseInt(s[1])-1,r=parseInt(s[2]),o=new Date(t.getFullYear(),n,r);return y(o)}const a=Date.parse(e);return isNaN(a)?null:y(new Date(a))}function x(e,t){const s=new Date(e);return s.setDate(s.getDate()+t),s}function y(e){const t=e.getFullYear(),s=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${s}-${a}`}function F(e){if(!e)return{title:"",dueDate:""};const t=/(?:^|\s)(오늘|금일|내일|명일|모레|어제|글피|today|tomorrow|yesterday)(?:까지|에|이|가|\s|$)/i,s=/(?:^|\s)((?:이번주|다음주)?\s*[월화수목금토일]요일)(?:까지|에|이|가|\s|$)/,a=new RegExp("(?<!\\d)(\\d{1,2}\\s*월\\s*\\d{1,2}\\s*일?)(?:까지|에|이|가|\\s|$)"),n=new RegExp("(?<!\\d)(\\d{1,2}[/\\-.]\\d{1,2})(?:일|요일|까지|에|이|가|\\s|$)"),r=new RegExp("(?<!\\d)(\\d{4}[/\\-.]\\d{1,2}[/\\-.]\\d{1,2})(?:까지|에|이|가|\\s|$)");let o="",c=e,d=c.match(r);if(d){const i=d[0].trim(),f=d[1].trim(),l=E(f);l&&(o=l,c=c.replace(i,"").trim())}if(!o&&(d=c.match(a)||c.match(n),d)){const i=d[0].trim(),f=d[1].trim(),l=E(f);l&&(o=l,c=c.replace(i,"").trim())}if(!o&&(d=c.match(s),d)){const i=d[0].trim(),f=d[1].trim(),l=E(f);l&&(o=l,c=c.replace(i,"").trim())}if(!o&&(d=c.match(t),d)){const i=d[0].trim(),f=d[1].trim(),l=E(f);l&&(o=l,c=c.replace(i,"").trim())}return c=c.replace(/\s+/g," ").trim(),c=c.replace(/^(까지|에|은|는|이|가|까지의)\s+/,""),c=c.replace(/\s+(까지|e|에|은|는|이|가|까지의)$/,""),c=c.trim(),{title:c,dueDate:o}}const M=document.getElementById("app-content"),U=document.getElementById("btn-board-view"),W=document.getElementById("btn-calendar-view"),Y=document.getElementById("btn-dashboard-view"),st=document.getElementById("btn-add-task");let b=null;function q(e){const t=e.currentView;U.classList.toggle("active",t==="board"),W.classList.toggle("active",t==="calendar"),Y.classList.toggle("active",t==="dashboard"),L(),t==="calendar"?(M.innerHTML=et(),ct()):t==="dashboard"?(M.innerHTML=at(),dt()):(M.innerHTML=tt(),ot(),it()),window.lucide&&window.lucide.createIcons(),nt()}u.subscribe(q);function D(e,t){b={type:e,key:t}}function w(){b=null}function nt(){if(!b)return;const{type:e,key:t}=b;if(e==="board-add"&&u.getCurrentView()==="board"){const s=document.querySelector(`.static-inline-add[data-status="${t}"]`),a=s&&s.querySelector(".static-title-input");a&&a.focus()}else if(e==="calendar-add"&&u.getCurrentView()==="calendar"){const s=document.querySelector(`.calendar-static-add-container[data-date="${t}"]`),a=s&&s.querySelector(".calendar-static-input");a&&a.focus()}}function L(){const e=document.querySelector(".move-menu");e&&e.remove()}function rt(e,t){L();const s=u.getTasks().find(i=>i.id===t);if(!s)return;const a=u.getCategories().filter(i=>i!==s.category);if(a.length===0){window.alert("옮길 다른 탭이 없어요. 먼저 위쪽 '+ 탭 추가'로 탭을 만드세요.");return}const n=document.createElement("div");n.className="move-menu";const r=document.createElement("div");r.className="move-menu-title",r.textContent="어느 탭으로 옮길까요?",n.appendChild(r),a.forEach(i=>{const f=document.createElement("button");f.className="move-menu-item",f.textContent=i,f.addEventListener("click",l=>{l.stopPropagation(),u.updateTask(t,{category:i}),L()}),n.appendChild(f)}),document.body.appendChild(n);const o=e.getBoundingClientRect();let d=window.scrollX+o.right-160;d<8&&(d=8),n.style.top=window.scrollY+o.bottom+6+"px",n.style.left=d+"px",setTimeout(()=>{document.addEventListener("click",function i(f){n.contains(f.target)||(L(),document.removeEventListener("click",i))})},0)}function ot(){document.querySelectorAll(".task-card").forEach(e=>{e.addEventListener("dragstart",t=>{if(e.querySelector("input")){t.preventDefault();return}t.dataTransfer.setData("text/plain",e.dataset.id),t.dataTransfer.effectAllowed="move",setTimeout(()=>e.classList.add("dragging"),0)}),e.addEventListener("dragend",()=>e.classList.remove("dragging"))}),document.querySelectorAll(".static-inline-add .static-title-input").forEach(e=>{const s=e.closest(".static-inline-add").dataset.status;e.addEventListener("focus",()=>D("board-add",s)),e.addEventListener("blur",()=>{setTimeout(()=>{b&&b.type==="board-add"&&b.key===s&&document.activeElement!==e&&w()},100)}),e.addEventListener("keydown",a=>{if(!(a.isComposing||a.keyCode===229))if(a.key==="Enter"){a.preventDefault();const n=e.value.trim();if(!n)return;D("board-add",s);const{title:r,dueDate:o}=F(n);if(!r)return;u.addTask(r,o)}else a.key==="Escape"&&(e.value="",e.blur(),w())})}),document.querySelectorAll(".inline-edit-title").forEach(e=>{e.addEventListener("dblclick",t=>{if(t.stopPropagation(),e.querySelector("input"))return;const s=e.dataset.id,a=u.getTasks().find(i=>i.id===s),n=a?a.title:e.textContent.trim(),r=document.createElement("input");r.type="text",r.className="form-input py-1 px-1.5 text-xs w-full",r.style.fontFamily="inherit",r.value=n,e.innerHTML="",e.appendChild(r),r.focus(),r.select();let o=!1;w();const c=()=>{if(o)return;o=!0;const i=r.value.trim();i&&i!==n?u.updateTask(s,{title:i}):e.textContent=n},d=()=>{o||(o=!0,e.textContent=n)};r.addEventListener("blur",c),r.addEventListener("keydown",i=>{i.isComposing||i.keyCode===229||(i.key==="Enter"?(i.preventDefault(),c()):i.key==="Escape"&&(i.preventDefault(),d()))})})}),document.querySelectorAll(".task-card-due").forEach(e=>{e.addEventListener("click",t=>{if(t.stopPropagation(),e.querySelector("input"))return;const s=e.dataset.id,a=e.querySelector(".due-text"),n=u.getTasks().find(l=>l.id===s),r=n?n.dueDate:a?a.textContent.trim():"",o=document.createElement("input");o.type="text",o.className="form-input py-0.5 px-1 text-[11px] font-mono",o.style.width="140px",o.style.display="inline-block",o.placeholder="예: 오늘, 내일, 6/17",o.value=r;const c=e.querySelector("i");c&&(c.style.display="none"),a&&(a.style.display="none"),e.appendChild(o),o.focus(),o.select();let d=!1;w();const i=()=>{if(d)return;d=!0;const l=o.value.trim();if(l){const g=E(l)||l;u.updateTask(s,{dueDate:g})}else u.updateTask(s,{dueDate:""})},f=()=>{d||(d=!0,c&&(c.style.display=""),a&&(a.style.display=""),o.remove())};o.addEventListener("blur",i),o.addEventListener("keydown",l=>{l.isComposing||l.keyCode===229||(l.key==="Enter"?(l.preventDefault(),i()):l.key==="Escape"&&(l.preventDefault(),f()))})})}),document.querySelectorAll(".delete-task-btn").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),u.deleteTask(e.dataset.id)})}),document.querySelectorAll(".reminder-check-btn").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation();const s=e.dataset.id,a=e.dataset.status==="done"?"todo":"done";u.updateTaskStatus(s,a)})}),document.querySelectorAll(".card-move-btn").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),rt(e,e.dataset.id)})})}function it(){document.querySelectorAll(".cat-tab").forEach(t=>{t.addEventListener("click",()=>u.setActiveCategory(t.dataset.cat))}),document.querySelectorAll(".cat-tab-wrap").forEach(t=>{const s=t.dataset.cat;t.addEventListener("dragover",a=>{a.preventDefault(),t.classList.add("tab-drop-over")}),t.addEventListener("dragleave",()=>t.classList.remove("tab-drop-over")),t.addEventListener("drop",a=>{a.preventDefault(),t.classList.remove("tab-drop-over");const n=a.dataTransfer.getData("text/plain");n&&s&&u.updateTask(n,{category:s})})}),document.querySelectorAll(".cat-tab-named").forEach(t=>{t.addEventListener("dblclick",s=>{if(s.stopPropagation(),s.preventDefault(),t.querySelector("input"))return;const a=t.dataset.cat,n=document.createElement("input");n.type="text",n.className="cat-tab-edit-input form-input",n.value=a,t.textContent="",t.appendChild(n),n.focus(),n.select();let r=!1;const o=c=>{if(r)return;r=!0;const d=n.value.trim();if(c&&d&&d!==a){const i=u.renameCategory(a,d);!i.ok&&i.message&&(window.alert(i.message),t.textContent=a)}else t.textContent=a};n.addEventListener("keydown",c=>{c.isComposing||c.keyCode===229||(c.key==="Enter"?(c.preventDefault(),o(!0)):c.key==="Escape"&&(c.preventDefault(),o(!1)))}),n.addEventListener("blur",()=>o(!0))})}),document.querySelectorAll(".cat-tab-delete").forEach(t=>{t.addEventListener("click",s=>{s.stopPropagation();const a=t.dataset.cat;if(!window.confirm(`'${a}' 탭을 삭제할까요?

이 탭의 할 일은 사라지지 않고 다른 탭으로 옮겨집니다.`))return;const r=u.deleteCategory(a);!r.ok&&r.message&&window.alert(r.message)})});const e=document.getElementById("cat-tab-add-btn");e&&e.addEventListener("click",()=>{if(document.getElementById("cat-tab-add-input"))return;const t=document.createElement("input");t.id="cat-tab-add-input",t.type="text",t.className="cat-tab-edit-input form-input",t.placeholder="새 탭 이름",e.parentNode.insertBefore(t,e),e.style.display="none",t.focus();let s=!1;const a=n=>{if(s)return;s=!0;const r=t.value.trim();if(t.parentNode&&t.remove(),e.style.display="",n&&r){const o=u.addCategory(r);!o.ok&&o.message&&window.alert(o.message)}};t.addEventListener("keydown",n=>{n.isComposing||n.keyCode===229||(n.key==="Enter"?(n.preventDefault(),a(!0)):n.key==="Escape"&&(n.preventDefault(),a(!1)))}),t.addEventListener("blur",()=>a(!0))})}function ct(){const e=document.getElementById("calendar-prev-btn"),t=document.getElementById("calendar-next-btn"),s=document.getElementById("calendar-today-btn");e&&t&&s&&(e.addEventListener("click",()=>{const a=u.getCurrentMonth();u.setCalendarMonth(new Date(a.getFullYear(),a.getMonth()-1,1))}),t.addEventListener("click",()=>{const a=u.getCurrentMonth();u.setCalendarMonth(new Date(a.getFullYear(),a.getMonth()+1,1))}),s.addEventListener("click",()=>u.setCalendarMonth(new Date))),document.querySelectorAll(".calendar-static-input").forEach(a=>{const r=a.closest(".calendar-static-add-container").dataset.date;a.addEventListener("focus",()=>D("calendar-add",r)),a.addEventListener("blur",()=>{setTimeout(()=>{b&&b.type==="calendar-add"&&b.key===r&&document.activeElement!==a&&w()},100)}),a.addEventListener("keydown",o=>{if(!(o.isComposing||o.keyCode===229))if(o.key==="Enter"){o.preventDefault();const c=a.value.trim();if(!c)return;D("calendar-add",r);const{title:d,dueDate:i}=F(c);if(!d)return;u.addTask(d,i||r)}else o.key==="Escape"&&(a.value="",a.blur(),w())})}),document.querySelectorAll(".calendar-inline-edit-pill").forEach(a=>{a.addEventListener("dblclick",n=>{if(n.stopPropagation(),a.querySelector("input"))return;const r=a.dataset.id,o=u.getTasks().find(g=>g.id===r),c=o?o.title:a.textContent.trim(),d=document.createElement("input");d.type="text",d.className="form-input py-0.5 px-1 text-[9px] w-full",d.style.height="16px",d.value=c,a.innerHTML="",a.appendChild(d),d.focus(),d.select();let i=!1;w();const f=()=>{if(i)return;i=!0;const g=d.value.trim();g&&g!==c?u.updateTask(r,{title:g}):u.notify()},l=()=>{i||(i=!0,u.notify())};d.addEventListener("blur",f),d.addEventListener("keydown",g=>{g.isComposing||g.keyCode===229||(g.key==="Enter"?(g.preventDefault(),f()):g.key==="Escape"&&(g.preventDefault(),l()))})})})}function dt(){const e=document.getElementById("dashboard-notepad");e&&e.addEventListener("input",t=>u.saveMemo(t.target.value)),document.querySelectorAll(".btn-quick-done").forEach(t=>{t.addEventListener("click",()=>u.updateTaskStatus(t.dataset.id,"done"))}),document.querySelectorAll(".dash-cat-link").forEach(t=>{t.addEventListener("click",()=>{u.setActiveCategory(t.dataset.cat),u.setView("board")})})}U.addEventListener("click",()=>u.setView("board"));W.addEventListener("click",()=>u.setView("calendar"));Y.addEventListener("click",()=>u.setView("dashboard"));st.addEventListener("click",()=>{u.getCurrentView()!=="board"?(u.setView("board"),setTimeout(R,50)):R()});function R(){const e=document.querySelector('.static-inline-add[data-status="todo"]'),t=e&&e.querySelector(".static-title-input");t&&(t.focus(),D("board-add","todo"))}const K=document.getElementById("btn-export-data"),H=document.getElementById("btn-import-data"),$=document.getElementById("import-file-input");K&&K.addEventListener("click",()=>{const e=u.exportData(),t=new Blob([e],{type:"application/json"}),s=URL.createObjectURL(t),a=document.createElement("a"),n=new Date().toISOString().split("T")[0];a.href=s,a.download=`졸업칸반-백업-${n}.json`,document.body.appendChild(a),a.click(),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(s),1e3)});H&&$&&(H.addEventListener("click",()=>$.click()),$.addEventListener("change",e=>{const t=e.target.files&&e.target.files[0];if(!t)return;const s=new FileReader;s.onload=a=>{const n=u.importData(a.target.result);n.ok?window.alert(`불러오기 완료! 할 일 ${n.count}개를 가져왔어요.`):window.alert(n.message),$.value=""},s.readAsText(t)}));document.addEventListener("DOMContentLoaded",()=>q(u.state));(document.readyState==="interactive"||document.readyState==="complete")&&q(u.state);
