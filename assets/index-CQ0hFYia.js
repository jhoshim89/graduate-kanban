...
- 탭(일상/개발)은 위쪽 '+ 탭 추가'로 얼마든지 만들 수 있어요.`,h="__all__";class tt{constructor(){this.tasksKey="graduate_kanban_tasks_v2",this.viewKey="graduate_kanban_view_v2",this.memoKey="graduate_kanban_memo_v2",this.catKey="graduate_kanban_categories_v2",this.activeCatKey="graduate_kanban_active_category_v2";const t=localStorage.getItem(this.tasksKey);let s=t?JSON.parse(t):Q;s=s.map(d=>{if(d.category)return d;const f=j[d.owner]||"일상";return{...d,category:f}});const a=localStorage.getItem(this.catKey);let n=a?JSON.parse(a):null;if(!n||!Array.isArray(n)||n.length===0){const d=[...new Set(s.map(f=>f.category).filter(Boolean))];n=[...new Set([...d,...O])]}const r=localStorage.getItem(this.viewKey)||"board",o=r.startsWith("board")?"board":r,i=localStorage.getItem(this.activeCatKey),c=i&&(i===h||n.includes(i))?i:h;this.state={tasks:s,categories:n,activeCategory:c,currentView:o,currentMonth:new Date,memo:localStorage.getItem(this.memoKey)||P},t||(this.saveTasksToLocalStorage(),localStorage.setItem(this.memoKey,P)),this.saveTasksToLocalStorage(),this.saveCategoriesToLocalStorage(),this.listeners=[]}saveTasksToLocalStorage(){localStorage.setItem(this.tasksKey,JSON.stringify(this.state.tasks))}saveViewToLocalStorage(){localStorage.setItem(this.viewKey,this.state.currentView)}saveCategoriesToLocalStorage(){localStorage.setItem(this.catKey,JSON.stringify(this.state.categories))}saveActiveCategoryToLocalStorage(){localStorage.setItem(this.activeCatKey,this.state.activeCategory)}getTasks(){return[...this.state.tasks]}getCategories(){return[...this.state.categories]}getActiveCategory(){return this.state.activeCategory}getVisibleTasks(){return this.state.activeCategory===h?[...this.state.tasks]:this.state.tasks.filter(t=>t.category===this.state.activeCategory)}getCurrentView(){return this.state.currentView}getCurrentMonth(){return this.state.currentMonth}getMemo(){return this.state.memo}setView(t){["board","calendar","dashboard"].includes(t)&&(this.state.currentView=t,this.saveViewToLocalStorage(),this.notify())}setActiveCategory(t){t!==h&&!this.state.categories.includes(t)||(this.state.activeCategory=t,this.saveActiveCategoryToLocalStorage(),this.notify())}addCategory(t){const s=(t||"").trim();return s?s===h||s==="전체"?{ok:!1,message:"'전체'는 기본 탭이라 추가할 수 없어요."}:this.state.categories.includes(s)?{ok:!1,message:"이미 있는 탭이에요."}:(this.state.categories.push(s),this.saveCategoriesToLocalStorage(),this.state.activeCategory=s,this.saveActiveCategoryToLocalStorage(),this.notify(),{ok:!0}):{ok:!1,message:"탭 이름을 입력해 주세요."}}deleteCategory(t){if(!this.state.categories.includes(t))return{ok:!1};if(this.state.categories.length<=1)return{ok:!1,message:"탭은 최소 한 개는 있어야 해요."};this.state.categories=this.state.categories.filter(a=>a!==t);const s=this.state.categories[0];return this.state.tasks=this.state.tasks.map(a=>a.category===t?{...a,category:s}:a),this.state.activeCategory===t&&(this.state.activeCategory=h,this.saveActiveCategoryToLocalStorage()),this.saveCategoriesToLocalStorage(),this.saveTasksToLocalStorage(),this.notify(),{ok:!0,movedTo:s}}renameCategory(t,s){const a=(s||"").trim();return a?this.state.categories.includes(t)?a===t?{ok:!0}:this.state.categories.includes(a)?{ok:!1,message:"이미 있는 탭 이름이에요."}:(this.state.categories=this.state.categories.map(n=>n===t?a:n),this.state.tasks=this.state.tasks.map(n=>n.category===t?{...n,category:a}:n),this.state.activeCategory===t&&(this.state.activeCategory=a,this.saveActiveCategoryToLocalStorage()),this.saveCategoriesToLocalStorage(),this.saveTasksToLocalStorage(),this.notify(),{ok:!0}):{ok:!1}:{ok:!1,message:"탭 이름을 입력해 주세요."}}setCalendarMonth(t){this.state.currentMonth=t,this.notify()}saveMemo(t){this.state.memo=t,localStorage.setItem(this.memoKey,t)}addTask(t,s="",a=null){const n=a||(this.state.activeCategory!==h?this.state.activeCategory:this.state.categories[0]||"일상"),r={id:`task-${Date.now()}-${Math.random().toString(36).substr(2,9)}`,title:t.trim(),dueDate:s||"",status:"todo",category:n};return this.state.tasks.push(r),this.saveTasksToLocalStorage(),this.notify(),r}updateTask(t,s){this.state.tasks=this.state.tasks.map(a=>a.id===t?{...a,...s}:a),this.saveTasksToLocalStorage(),this.notify()}updateTaskStatus(t,s){["todo","inprogress","done"].includes(s)&&this.updateTask(t,{status:s})}deleteTask(t){this.state.tasks=this.state.tasks.filter(s=>s.id!==t),this.saveTasksToLocalStorage(),this.notify()}exportData(){return JSON.stringify({_app:"graduate-kanban",_version:3,tasks:this.state.tasks,categories:this.state.categories,memo:this.state.memo,currentView:this.state.currentView},null,2)}importData(t){let s;try{s=JSON.parse(t)}catch{return{ok:!1,message:"파일을 읽을 수 없어요. 올바른 백업 파일인지 확인해 주세요."}}if(!s||!Array.isArray(s.tasks))return{ok:!1,message:"이 칸반 백업 파일이 아닌 것 같아요."};const a=s.tasks.map(r=>r.category?r:{...r,category:j[r.owner]||"일상"});let n;if(Array.isArray(s.categories)&&s.categories.length>0)n=[...new Set(s.categories)];else{const r=[...new Set(a.map(o=>o.category).filter(Boolean))];n=[...new Set([...r,...O])]}return this.state.tasks=a,this.state.categories=n,n.includes(this.state.activeCategory)||(this.state.activeCategory=h,this.saveActiveCategoryToLocalStorage()),typeof s.memo=="string"&&(this.state.memo=s.memo),this.saveTasksToLocalStorage(),this.saveCategoriesToLocalStorage(),localStorage.setItem(this.memoKey,this.state.memo),this.notify(),{ok:!0,count:a.length}}subscribe(t){return this.listeners.push(t),()=>{this.listeners=this.listeners.filter(s=>s!==t)}}notify(){this.listeners.forEach(t=>t(this.state))}}const u=new tt;function U(e){if(!e)return{class:"due-none",text:"기한 없음"};const t=new Date;t.setHours(0,0,0,0);const s=new Date(e);s.setHours(0,0,0,0);const a=s-t,n=Math.ceil(a/(1e3*60*60*24));if(n<0)return{class:"due-overdue",text:`지연 D+${Math.abs(n)}`};if(n===0)return{class:"due-overdue",text:"오늘"};if(n===1)return{class:"due-soon",text:"내일"};if(n===2)return{class:"due-soon",text:"모레"};{const r=e.split("-");return{class:"due-normal",text:`${r.length===3?`${parseInt(r[1])}/${parseInt(r[2])}`:e} (D-${n})`}}}function et(){const e=u.getCategories(),t=u.getActiveCategory();return`
  ...
  `}function R(e,t){const s=e.status==="done",a=U(e.dueDate),n=s?"task-card-title inline-edit-title line-through":"task-card-title inline-edit-title",r=s?"text-mute":a.class,o=s?"check-circle-2":"circle",i=s?"text-violet":"text-mute",c=N(e.title),d=t&&e.category?`<span class="card-category-badge">${N(e.category)}</span>`:"";return`
  ...
  `}function at(){const e=u.getVisibleTasks(),t=u.getActiveCategory()===h,s=e.filter(r=>r.status!=="done"),a=e.filter(r=>r.status==="done"),n=s.length===0&&a.length===0?'<div class="empty-state list-empty"><span>이 탭에 할 일이 없어요. 아래에 입력해서 추가하세요.</span></div>':"";return`
    ${et()}
  ...
  `}function N(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function st(){const e=u.getCurrentMonth(),t=e.getFullYear(),s=e.getMonth(),a=u.getTasks(),n=new Date(t,s,1).getDay(),r=new Date(t,s+1,0).getDate(),o=new Date(t,s,0).getDate(),i=new Date;i.setHours(0,0,0,0);const c=[];for(let p=n-1;p>=0;p--){const v=new Date(t,s-1,o-p);c.push({date:v,isCurrentMonth:!1,dayNumber:o-p})}for(let p=1;p<=r;p++){const v=new Date(t,s,p),C=v.toDateString()===i.toDateString();c.push({date:v,isCurrentMonth:!0,isToday:C,dayNumber:p})}const f=42-c.length;for(let p=1;p<=f;p++){const v=new Date(t,s+1,p);c.push({date:v,isCurrentMonth:!1,dayNumber:p})}return`
    <div class="calendar-view">
      ...
      <div class="calendar-grid">
        ${c.map(p=>{const v=p.date.toISOString().split("T")[0],C=a.filter(m=>m.dueDate===v);return`
  ...
  `}function k(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function E(e){if(!e)return null;e=e.trim().toLowerCase();const t=new Date;if(t.setHours(0,0,0,0),/^(오늘|금일|today|d-day|dday)$/.test(e))return y(t);if(/^(내일|명일|tomorrow)$/.test(e))return y(x(t,1));if(/^(모레|내일모레|글피)$/.test(e))return y(x(t,2));if(/^(어제|작일|yesterday)$/.test(e))return y(x(t,-1));let s=e.match(/^(\d+)\s*일\s*(뒤|후|후에|이후)$/);if(s||(s=e.match(/^(\d+)\s*days?\s*(after|later)$/),s))return y(x(t,parseInt(s[1])));if(s=e.match(/^(\d+)\s*일\s*(전|전에|이전)$/),s)return y(x(t,-parseInt(s[1])));if(s=e.match(/^(이번주|다음주)?\s*(월|화|수|목|금|토|일)요일?$/),s){const n=s[1]==="다음주",r=s[2],i=["일","월","화","수","목","금","토"].indexOf(r),c=t.getDay();let d=i-c;return(n||d<=0)&&(d+=7),y(x(t,d))}if(s=e.match(/^(\d{1,2})\s*[월/\-.]\s*(\d{1,2})\s*일?$/),s){const n=parseInt(s[1])-1,r=parseInt(s[2]),o=new Date(t.getFullYear(),n,r);return y(o)}const a=Date.parse(e);return isNaN(a)?null:y(new Date(a))}function x(e,t){const s=new Date(e);return s.setDate(s.getDate()+t),s}function y(e){const t=e.getFullYear(),s=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${s}-${a}`}function W(e){if(!e)return{title:"",dueDate:""};const t=/(?:^|\s)(오늘|금일|내일|명일|모레|어제|글피|today|tomorrow|yesterday)(?:까지|에|이|가|\s|$)/i,s=/(?:^|\s)((?:이번주|다음주)?\s*[월화수목금토일]요일)(?:까지|에|이|가|\s|$)/,a=new RegExp("(?<!\\d)(\\d{1,2}\\s*월\\s*\\d{1,2}\\s*일?)(?:까지|에|이|가|\\s|$)"),n=new RegExp("(?<!\\d)(\\d{1,2}[/\\-.]\\d{1,2})(?:일|요일|까지|에|이|가|\\s|$)"),r=new RegExp("(?<!\\d)(\\d{4}[/\\-.]\\d{1,2}[/\\-.]\\d{1,2})(?:까지|에|이|가|\\s|$)");let o="",i=e,c=i.match(r);if(c){const d=c[0].trim(),f=c[1].trim(),l=E(f);l&&(o=l,i=i.replace(d,"").trim())}if(!o&&(c=i.match(a)||i.match(n),c)){const d=c[0].trim(),f=c[1].trim(),l=E(f);l&&(o=l,i=i.replace(d,"").trim())}if(!o&&(c=i.match(s),c)){const d=c[0].trim(),f=c[1].trim(),l=E(f);l&&(o=l,i=i.replace(d,"").trim())}if(!o&&(c=i.match(t),c)){const d=c[0].trim(),f=c[1].trim(),l=E(f);l&&(o=l,i=i.replace(d,"").trim())}return i=i.replace(/\s+/g," ").trim(),i=i.replace(/^(까지|에|은|는|이|가|까지의)\s+/,""),i=i.replace(/\s+(까지|e|에|은|는|이|가|까지의)$/,""),i=i.trim(),{title:i,dueDate:o}}const A=document.getElementById("app-content"),Y=document.getElementById("btn-board-view"),z=document.getElementById("btn-calendar-view"),G=document.getElementById("btn-dashboard-view"),rt=document.getElementById("btn-add-task");let b=null;function B(e){const t=e.currentView;Y.classList.toggle("active",t==="board"),z.classList.toggle("active",t==="calendar"),G.classList.toggle("active",t==="dashboard"),S(),t==="calendar"?(A.innerHTML=st(),ut()):t==="dashboard"?(A.innerHTML=nt(),ft()):(A.innerHTML=at(),ct(),lt()),window.lucide&&window.lucide.createIcons(),ot()}u.subscribe(B);function D(e,t){b={type:e,key:t}}function w(){b=null}function ot(){if(!b)return;const{type:e,key:t}=b;if(e==="board-add"&&u.getCurrentView()==="board"){const s=document.querySelector(`.static-inline-add[data-status="${t}"]`),a=s&&s.querySelector(".static-title-input");a&&a.focus()}else if(e==="calendar-add"&&u.getCurrentView()==="calendar"){const s=document.querySelector(`.calendar-static-add-container[data-date="${t}"]`),a=s&&s.querySelector(".calendar-static-input");a&&a.focus()}}function S(){const e=document.querySelector(".move-menu");e&&e.remove()}let I=null;function it(e){q();const t=u.getTasks().find(r=>r.id===e);if(!t)return;const s=u.getCategories(),a=document.createElement("div");a.className="drag-drop-panel";const n=document.createElement("div");n.className="drag-drop-panel-title",n.textContent="여기로 끌어 놓으세요",a.appendChild(n),s.forEach(r=>{const o=document.createElement("div");o.className="drag-drop-zone"+(r===t.category?" is-current":""),o.textContent=r===t.category?`${r} (현재)`:r,o.addEventListener("dragenter",i=>{i.preventDefault(),o.classList.add("over")}),o.addEventListener("dragover",i=>{i.preventDefault(),i.dataTransfer.dropEffect="move",o.classList.add("over")}),o.addEventListener("dragleave",()=>o.classList.remove("over")),o.addEventListener("drop",i=>{i.preventDefault(),i.stopPropagation();const c=i.dataTransfer.getData("text/plain")||e;u.updateTask(c,{category:r}),q()}),a.appendChild(o)}),document.body.appendChild(a),I=a}function q(){I&&(I.remove(),I=null)}function dt(e,t){S();const s=u.getTasks().find(d=>d.id===t);if(!s)return;const a=u.getCategories().filter(d=>d!==s.category);if(a.length===0){window.alert("옮길 다른 탭이 없어요. 먼저 위쪽 '+ 탭 추가'로 탭을 만드세요.");return}const n=document.createElement("div");n.className="move-menu";const r=document.createElement("div");r.className="move-menu-title",r.textContent="어느 탭으로 옮길까요?",n.appendChild(r),a.forEach(d=>{const f=document.createElement("button");f.className="move-menu-item",f.textContent=d,f.addEventListener("click",l=>{l.stopPropagation(),u.updateTask(t,{category:d}),S()}),n.appendChild(f)}),document.body.appendChild(n);const o=e.getBoundingClientRect();let c=window.scrollX+o.right-160;c<8&&(c=8),n.style.top=window.scrollY+o.bottom+6+"px",n.style.left=c+"px",setTimeout(()=>{document.addEventListener("click",function d(f){n.contains(f.target)||(S(),document.removeEventListener("click",d))})},0)}function ct(){document.querySelectorAll(".task-card").forEach(e=>{e.addEventListener("dragstart",t=>{if(e.querySelector("input")){t.preventDefault();return}t.dataTransfer.setData("text/plain",e.dataset.id),t.dataTransfer.effectAllowed="move",setTimeout(()=>e.classList.add("dragging"),0),it(e.dataset.id)}),e.addEventListener("dragend",()=>{e.classList.remove("dragging"),q()})}),document.querySelectorAll(".static-inline-add .static-title-input").forEach(e=>{const s=e.closest(".static-inline-add").dataset.status;e.addEventListener("focus",()=>D("board-add",s)),e.addEventListener("blur",()=>{setTimeout(()=>{b&&b.type==="board-add"&&b.key===s&&document.activeElement!==e&&w()},100)}),e.addEventListener("keydown",a=>{if(!(a.isComposing||a.keyCode===229))if(a.key==="Enter"){a.preventDefault();const n=e.value.trim();if(!n)return;D("board-add",s);const{title:r,dueDate:o}=W(n);if(!r)return;u.addTask(r,o)}else a.key==="Escape"&&(e.value="",e.blur(),w())})}),document.querySelectorAll(".inline-edit-title").forEach(e=>{e.addEventListener("dblclick",t=>{if(t.stopPropagation(),e.querySelector("input"))return;const s=e.dataset.id,a=u.getTasks().find(d=>d.id===s),n=a?a.title:e.textContent.trim(),r=document.createElement("input");r.type="text",r.className="form-input py-1 px-1.5 text-xs w-full",r.style.fontFamily="inherit",r.value=n,e.innerHTML="",e.appendChild(r),r.focus(),r.select();let o=!1;w();const i=()=>{if(o)return;o=!0;const d=r.value.trim();d&&d!==n?u.updateTask(s,{title:d}):e.textContent=n},c=()=>{o||(o=!0,e.textContent=n)};r.addEventListener("blur",i),r.addEventListener("keydown",d=>{d.isComposing||d.keyCode===229||(d.key==="Enter"?(d.preventDefault(),i()):d.key==="Escape"&&(d.preventDefault(),c()))})})}),document.querySelectorAll(".task-card-due").forEach(e=>{e.addEventListener("click",t=>{if(t.stopPropagation(),e.querySelector("input"))return;const s=e.dataset.id,a=e.querySelector(".due-text"),n=u.getTasks().find(l=>l.id===s),r=n?n.dueDate:a?a.textContent.trim():"",o=document.createElement("input");o.type="text",o.className="form-input py-0.5 px-1 text-[11px] font-mono",o.style.width="140px",o.style.display="inline-block",o.placeholder="예: 오늘, 내일, 6/17",o.value=r;const i=e.querySelector("i");i&&(i.style.display="none"),a&&(a.style.display="none"),e.appendChild(o),o.focus(),o.select();let c=!1;w();const d=()=>{if(c)return;c=!0;const l=o.value.trim();if(l){const g=E(l)||l;u.updateTask(s,{dueDate:g})}else u.updateTask(s,{dueDate:""})},f=()=>{c||(c=!0,i&&(i.style.display=""),a&&(a.style.display=""),o.remove())};o.addEventListener("blur",d),o.addEventListener("keydown",l=>{l.isComposing||l.keyCode===229||(l.key==="Enter"?(l.preventDefault(),d()):l.key==="Escape"&&(l.preventDefault(),f()))})})}),document.querySelectorAll(".delete-task-btn").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),u.deleteTask(e.dataset.id)})}),document.querySelectorAll(".reminder-check-btn").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation();const s=e.dataset.id,a=e.dataset.status==="done"?"todo":"done";u.updateTaskStatus(s,a)})}),document.querySelectorAll(".card-move-btn").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),dt(e,e.dataset.id)})})}function lt(){document.querySelectorAll(".cat-tab").forEach(t=>{t.addEventListener("click",()=>u.setActiveCategory(t.dataset.cat))}),document.querySelectorAll(".cat-tab-wrap").forEach(t=>{const s=t.dataset.cat;t.addEventListener("dragover",a=>{a.preventDefault(),t.classList.add("tab-drop-over")}),t.addEventListener("dragleave",()=>t.classList.remove("tab-drop-over")),t.addEventListener("drop",a=>{a.preventDefault(),t.classList.remove("tab-drop-over");const n=a.dataTransfer.getData("text/plain");n&&s&&u.updateTask(n,{category:s})})}),document.querySelectorAll(".cat-tab-named").forEach(t=>{t.addEventListener("dblclick",s=>{if(s.stopPropagation(),s.preventDefault(),t.querySelector("input"))return;const a=t.dataset.cat,n=document.createElement("input");n.type="text",n.className="cat-tab-edit-input form-input",n.value=a,t.textContent="",t.appendChild(n),n.focus(),n.select();let r=!1;const o=i=>{if(r)return;r=!0;const c=n.value.trim();if(i&&c&&c!==a){const d=u.renameCategory(a,c);!d.ok&&d.message&&(window.alert(d.message),t.textContent=a)}else t.textContent=a};n.addEventListener("keydown",i=>{i.isComposing||i.keyCode===229||(i.key==="Enter"?(i.preventDefault(),o(!0)):i.key==="Escape"&&(i.preventDefault(),o(!1)))}),n.addEventListener("blur",()=>o(!0))})}),document.querySelectorAll(".cat-tab-delete").forEach(t=>{t.addEventListener("click",s=>{s.stopPropagation();const a=t.dataset.cat;if(!window.confirm(`'${a}' 탭을 삭제할까요?

이 탭의 할 일은 사라지지 않고 다른 탭으로 옮겨집니다.`))return;const r=u.deleteCategory(a);!r.ok&&r.message&&window.alert(r.message)})});const e=document.getElementById("cat-tab-add-btn");e&&e.addEventListener("click",()=>{if(document.getElementById("cat-tab-add-input"))return;const t=document.createElement("input");t.id="cat-tab-add-input",t.type="text",t.className="cat-tab-edit-input form-input",t.placeholder="새 탭 이름",e.parentNode.insertBefore(t,e),e.style.display="none",t.focus();let s=!1;const a=n=>{if(s)return;s=!0;const r=t.value.trim();if(t.parentNode&&t.remove(),e.style.display="",n&&r){const o=u.addCategory(r);!o.ok&&o.message&&window.alert(o.message)}};t.addEventListener("keydown",n=>{n.isComposing||n.keyCode===229||(n.key==="Enter"?(n.preventDefault(),a(!0)):n.key==="Escape"&&(n.preventDefault(),a(!1)))}),t.addEventListener("blur",()=>a(!0))})}function ut(){const e=document.getElementById("calendar-prev-btn"),t=document.getElementById("calendar-next-btn"),s=document.getElementById("calendar-today-btn");e&&t&&s&&(e.addEventListener("click",()=>{const a=u.getCurrentMonth();u.setCalendarMonth(new Date(a.getFullYear(),a.getMonth()-1,1))}),t.addEventListener("click",()=>{const a=u.getCurrentMonth();u.setCalendarMonth(new Date(a.getFullYear(),a.getMonth()+1,1))}),s.addEventListener("click",()=>u.setCalendarMonth(new Date))),document.querySelectorAll(".calendar-static-input").forEach(a=>{const r=a.closest(".calendar-static-add-container").dataset.date;a.addEventListener("focus",()=>D("calendar-add",r)),a.addEventListener("blur",()=>{setTimeout(()=>{b&&b.type==="calendar-add"&&b.key===r&&document.activeElement!==a&&w()},100)}),a.addEventListener("keydown",o=>{if(!(o.isComposing||o.keyCode===229))if(o.key==="Enter"){o.preventDefault();const i=a.value.trim();if(!i)return;D("calendar-add",r);const{title:c,dueDate:d}=W(i);if(!c)return;u.addTask(c,d||r)}else o.key==="Escape"&&(a.value="",a.blur(),w())})}),document.querySelectorAll(".calendar-inline-edit-pill").forEach(a=>{a.addEventListener("dblclick",n=>{if(n.stopPropagation(),a.querySelector("input"))return;const r=a.dataset.id,o=u.getTasks().find(g=>g.id===r),i=o?o.title:a.textContent.trim(),c=document.createElement("input");c.type="text",c.className="form-input py-0.5 px-1 text-[9px] w-full",c.style.height="16px",c.value=i,a.innerHTML="",a.appendChild(c),c.focus(),c.select();let d=!1;w();const f=()=>{if(d)return;d=!0;const g=c.value.trim();g&&g!==i?u.updateTask(r,{title:g}):u.notify()},l=()=>{d||(d=!0,u.notify())};c.addEventListener("blur",f),c.addEventListener("keydown",g=>{g.isComposing||g.keyCode===229||(g.key==="Enter"?(g.preventDefault(),f()):g.key==="Escape"&&(g.preventDefault(),l()))})})})}function ft(){const e=document.getElementById("dashboard-notepad");e&&e.addEventListener("input",t=>u.saveMemo(t.target.value)),document.querySelectorAll(".btn-quick-done").forEach(t=>{t.addEventListener("click",()=>u.updateTaskStatus(t.dataset.id,"done"))}),document.querySelectorAll(".dash-cat-link").forEach(t=>{t.addEventListener("click",()=>{u.setActiveCategory(t.dataset.cat),u.setView("board")})})}Y.addEventListener("click",()=>u.setView("board"));z.addEventListener("click",()=>u.setView("calendar"));G.addEventListener("click",()=>u.setView("dashboard"));rt.addEventListener("click",()=>{u.getCurrentView()!=="board"?(u.setView("board"),setTimeout(K,50)):K()});function K(){const e=document.querySelector('.static-inline-add[data-status="todo"]'),t=e&&e.querySelector(".static-title-input");t&&(t.focus(),D("board-add","todo"))}const H=document.getElementById("btn-export-data"),F=document.getElementById("btn-import-data"),$=document.getElementById("import-file-input");H&&H.addEventListener("click",()=>{const e=u.exportData(),t=new Blob([e],{type:"application/json"}),s=URL.createObjectURL(t),a=document.createElement("a"),n=new Date().toISOString().split("T")[0];a.href=s,a.download=`졸업칸반-백업-${n}.json`,document.body.appendChild(a),a.click(),document.body.removeChild(a),setTimeout(()=>URL.revokeObjectURL(s),1e3)});F&&$&&(F.addEventListener("click",()=>$.click()),$.addEventListener("change",e=>{const t=e.target.files&&e.target.files[0];if(!t)return;const s=new FileReader;s.onload=a=>{const n=u.importData(a.target.result);n.ok?window.alert(`불러오기 완료! 할 일 ${n.count}개를 가져왔어요.`):window.alert(n.message),$.value=""},s.readAsText(t)}));document.addEventListener("DOMContentLoaded",()=>B(u.state));(document.readyState==="interactive"||document.readyState==="complete")&&B(u.state);
